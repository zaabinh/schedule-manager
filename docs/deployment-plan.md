...
- Public origins: ưu tiên sibling custom domains như `app.school.edu.vn` và `api.school.edu.vn`. Domain mặc định khác site của Vercel/Render phải đi qua Vercel same-origin rewrite; browser không gọi Render trực tiếp vì session cookie là `SameSite=Lax`.
