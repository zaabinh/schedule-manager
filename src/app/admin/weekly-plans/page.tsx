...
export default function WeeklyPlansPage() {
  ...
  return <><PageHeader eyebrow={selectedYear ? `Năm học ${selectedYear.name}` : "Lịch năm học"} title="Kế hoạch tuần" description="Tạo DRAFT mới hoặc sao chép nội dung nền từ tuần trước." actions={<label><span className="sr-only">Năm học</span><select className="field" value={effectiveYear} onChange={(event) => setYearId(event.target.value)}>{years.map((year) => <option key={year.id} value={year.id}>{year.name}{year.isActive ? " · đang dùng" : ""}</option>)}</select></label>}/>
