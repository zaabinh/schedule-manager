...
- GitHub Actions baseline: workflow `ci` run [32271324894](https://github.com/zaabinh/schedule-manager/actions/runs/32271324894), attempt 2, PASS đủ năm job `frontend`, `backend`, `browser-e2e`, `performance-smoke` và `security-baseline` cho commit `04731dc`. Workflow hiện thêm timeout hữu hạn, log Docker khi E2E/security thất bại và job `production-provisioning`; evidence GitHub của thay đổi này phải PASS trước khi gắn release tag.
